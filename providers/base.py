...
class IPASourceProvider:
    ...
    def download_and_inspect_ipa(self, url: str, expected_bundle_id: str, expected_version: str) -> Dict[str, Any]:
                    ...
                    try:
                        ...
                        if status == 429 or status >= 500:
                            if attempt < self.max_retries:
                                ...
                                logger.warning("HTTP %s от %s; повтор %s/%s через %.1fs", status, clean_url, attempt, self.max_retries, delay)
                                response.close()
                                time.sleep(delay)
                                continue
                            response.close()
